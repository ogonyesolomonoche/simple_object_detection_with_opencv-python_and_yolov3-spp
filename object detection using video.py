...
while cap.isOpened():
    ...
    # a blob is created from the video frame and then passes through the network
    ...
    cv2.imshow("Object Detection", frame) # output video is displayed
    if cv2.waitKey(1) & 0xFF == ord('s'): # key S is used to break the loop
        break

cap.release() # video capture is released
